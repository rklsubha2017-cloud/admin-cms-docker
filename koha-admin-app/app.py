...
def handle_conn(data):
    ...
    #Update for CMS API
    cms_url = os.environ.get("CMS_VAULT_URL", "/api/vault/auto-ingest")
    cms_token = os.environ.get("CMS_JWT_TOKEN", "")

    # Instantiate a NEW Manager for this specific user
    new_ssh = SSHManager(scoped_emit, cms_url, cms_token)
...
def handle_task(data):
	...
	#update for CMS API
        socketio.start_background_task(ssh.get_deep_stats, args)
        ...
        ssh.configure_firewall(args) # Update for CMS APIy
    ...
    elif task == 'extract_vault': 
        socketio.start_background_task(ssh.extract_full_vault, args)
        
    elif task == 'push_vault': 
        socketio.start_background_task(ssh.push_to_cms, args.get('client_code'), args.get('payload'))
